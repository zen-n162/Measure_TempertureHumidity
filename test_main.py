...
#csv_file
csv_file_name = '/home/zen/Documents/Geek/TemperatureHumidity/test_temp_humi.csv'
...
def create_csv(file_name, datas):
    with open(file_name, 'a') as input_f:
        ...
        # 毎日1時頃に更新されるデータなので、２時に取得
        if((tdatetime.hour == 2) and (0 <= tdatetime.minute < 20)): # 毎日1時頃に更新されるデータなので、２時に取得
...
while True:
    try:
        ...
        while True:
            ...
            #humi, temp = Adafruit_DHT.read_retry(device, PIN)
            #異常な値なら再取得
